...
def fazer_login(
    ...
    if not usuario:
        return templates.TemplateResponse(request=request, name="login.html", context={"request": request, "erro": "Email ou senha inválidos!"})
    
    # Cria cookie com o ID do usuário
...
def criar_admin(db: Session = Depends(get_db)):
    ...
    criar_usuario(db, "Administrador", "admin@cafebii.com", "admin123", "GERENTE")
